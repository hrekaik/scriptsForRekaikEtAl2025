...
# finally a column "Well.10x" needs to be setup
# if 2 CellPlex or regular 10x experiments were done label them 1, 2, ...
# in summary the columns needed are: Sample.ID, RDS, Well.10x
...
samples.used <- 1:13
desiredName.for.RDS <- "combined_AlltimewtvsBADC"
#####
...
all.sorted <- paste0(metadata.sample$RDS[match(samples.used, metadata.sample$Sample.ID)])
...
if (!file.exists(file.path(RDSfolder, nameRDS))) {
  for (my.sample in all.sorted) {
    cat(paste0("loading ",my.sample, "\n"))
    if (!file.exists(file.path(RDSfolder, paste0(my.sample, ".RDS")))) {
      stop("The RDS file does not exists, run step1 or fix the metadata csv")
    }
    seurat.all.sorted.subset[[my.sample]] <- readRDS(file.path(RDSfolder, paste0(my.sample, ".RDS")))
  ...
  # If desired, set the factor for ordering sample by metadata specifically
  # combined.seurat[["Dissection"]] <- factor(combined.seurat[[]][["Dissection"]],
  #                                           levels = c("Anterior", "Posterior","Whole"))
  
  #Normalize Data, find vairable features, scale and regress cell cycle score
  ...
  if (length(unique(metadata.sample$Well.10x[match(samples.used , metadata.sample$Sample.ID)])) > 1) {
    ...
    average.expression <- rowMeans(AverageExpression(combined.seurat, group.by = "Well.10x")[[1]])
