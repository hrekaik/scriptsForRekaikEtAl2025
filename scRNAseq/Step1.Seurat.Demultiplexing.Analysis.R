...
output.directory <- "../../Code_figures/outputs/scRNAseq/"
...
metadata.file <- "../../Code_figures/scRNAseq/metadata.csv"
...
CellCycleScore.from.seuratObject <- function(seurat.object, output.metrics, s.genes, g2m.genes) {
  ...
  ggsave(filename = paste0(output.metrics, Project(seurat.object), ".RNA.QC.pre-subset.pdf"),
         VlnPlot(seurat.object,
                 features = c("nFeature_RNA",
                              ...
                              "percent.mt"),
                 ncol = 3), device = "pdf")
  # ---- Cell counts BEFORE subset ----
  n_cells_pre <- ncol(seurat.object)
  ...
  # ---- Cell counts AFTER subset ----
  n_cells_post <- ncol(seurat.object.subset)
  # ---- Save counts to txt file ----
  cell_count_file <- paste0(output.metrics, Project(seurat.object), ".cell_counts_pre_post_subset.txt")
  lines_out <- c(paste0("Project: ", Project(seurat.object)),
                 paste0("Cells before subset: ", n_cells_pre),
                 paste0("Cells after subset: ", n_cells_post))
  writeLines(lines_out, con = cell_count_file)
...
dir.create(output.metrics, showWarnings = FALSE, recursive = TRUE)
...
for (mydirectory in all.directory) {
  ...
  if (is.na(all.meta[1, "Multiplexing"])) {
    ...
    if (!file.exists(file.path(RDSfolder, paste0(all.meta$RDS, ".RDS")))) {
      ...
      # Save RDS
      saveRDS(seurat.object.subset, file = file.path(RDSfolder, paste0(all.meta$RDS, ".RDS")))
    ...
    if (!file.exists(file.path(RDSfolder, paste0(mydirectory, ".RDS")))) {
      ...
      saveRDS(seurat.object.clean.subset, file = file.path(RDSfolder, paste0(mydirectory, ".RDS")))
      ...
      for (sample in names(split.seurat.by.sample)) {
        ...
        saveRDS(split.seurat.by.sample[[sample]], file.path(RDSfolder, paste0(mydirectory, "_", sample, ".RDS")))
