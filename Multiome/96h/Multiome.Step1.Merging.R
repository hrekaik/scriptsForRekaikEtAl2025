...
hoxa.b.c.d.genes.file <- "scriptsForRekaikEtAl2025/outputs/general/delBADC_genes.txt"

setwd(wd)
dir.create(plot.dir, showWarnings = FALSE)

counts_wt <- Read10X("inputs_galaxy/wt/")
fragpath_wt <- "inputs_galaxy/Sorted_Fragments/wt.bgzip"

counts_BADC <- Read10X("inputs_galaxy/BADC/")
fragpath_BADC <- "inputs_galaxy/Sorted_Fragments/BADC.bgzip"

# get unique peaks from MACS2
unique_peaks <- import("inputs_galaxy/merged_peaks.bed")
...
annotation <- import.gff("inputs_galaxy/custom_mm10.gtf")
...
# find clusters
combined.seurat <- FindClusters(combined.seurat, graph.name = "wknn")
combined.seurat <- FindClusters(combined.seurat, graph.name = "wknn", resolution = 0.6)
combined.seurat <- FindClusters(combined.seurat, graph.name = "wknn", resolution = 0.5)
DimPlot(combined.seurat) +
  DimPlot(combined.seurat, group.by = "Genotype")
ggsave(file.path(plot.dir, "Clustering.pdf"), width = 8, height = 4)

saveRDS(combined.seurat, "combined_Multiome.RDS")
